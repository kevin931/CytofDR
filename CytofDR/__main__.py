...
from CytofDR import dr, metric
from CytofDR.fileio import FileIO
from CytofDR.util import DownSample, Annoy
...
parser.add_argument("--build_annoy", action="store_true",
                        help="Build Annoy model.")
parser.add_argument("--split_train", type=float, action="store",
                        help="Train-test split of input data and save to output directory.")
parser.add_argument("--downsample", type=int, action="store",
                        help="Downsample input file and embedding with n.")

# Methods: For all modes
parser.add_argument("-m", "--methods", nargs="+", action="store",
...
parser.add_argument("--transform", nargs="+", action="store",
...
parser.add_argument("--delim", type=str, action="store", default="\t",
                        help="File delimiter.")
parser.add_argument("--file_col_names", action="store_true",
                        help="Whether file's first row is names.")
parser.add_argument("--file_drop_col", type=int, nargs="+", action="store",
                        help="Columns to drop while reading files.")
parser.add_argument("--add_sample_index", action="store_true",
...
parser.add_argument("--no_new_dir", action="store_true",
                        help="Save results directly in -o without creating new directory.")
parser.add_argument("--save_embedding_colnames", action="store_true",
                        help="Save column names for embedding after DR.")
# File IO: Embedding and labels
parser.add_argument("--embedding", nargs="+", action="store",
                        help="Load embedding from directory or file path.")
parser.add_argument("--embedding_col_names", action="store_true",
                        help="Whether embedding's first row is names.")
parser.add_argument("--embedding_drop_col", type=int, nargs="+", action="store",
                        help="Columns to drop while reading files.")
parser.add_argument("--labels", action="store",
                        help="Path to pre-classified labels.")
parser.add_argument("--embedding_labels", action="store",
                        help="Path to pre-classified labels of embedding.")
parser.add_argument("--labels_col_names", action="store_true",
                        help="Whether embedding's first row is names.")
parser.add_argument("--labels_drop_col", type=int, nargs="+", action="store",
                        help="Columns of label to be dropped.")
# File IO: Comparison EMbedding and Labels
parser.add_argument("--comparison_file", nargs="+", action="store",
                        help="Load comparison file from directory or file path.")
parser.add_argument("--comparison_file_col_names", action="store_true",
                        help="Whether comparison file's first row is names.")
parser.add_argument("--comparison_file_drop_col", type=int, nargs="+", action="store",
                        help="Columns to drop while reading comparison file.")
parser.add_argument("--comparison_labels", nargs="+", action="store",
                        help="Path to pre-classified comparison labels of embedding.")
parser.add_argument("--comparison_labels_col_names", action="store_true",
                        help="Whether comparison embedding labels' first row is names.")
parser.add_argument("--comparison_labels_drop_col", type=int, nargs="+", action="store",
                        help="Columns of comparison label to be dropped.")
parser.add_argument("--comparison_classes", type=str, nargs="+", action="store",
...
# DR Evalation
parser.add_argument("--eval_k_neighbors", type=int, action="store", default=100,
                        help="Number of neighbors for local structure preservation metrics.")
parser.add_argument("--downsample_index_files",nargs="+", action="store",
                        help="File paths or directory path to saved downsample indicies as tsv.")
parser.add_argument("--file_annoy", type=str, action="store",
                        help="Path to the file's Annoy model.")
parser.add_argument("--pwd_metric", type=str, action="store", default="PCD",
                        help="Pairwise distace implementation: Full pairwise distance or approximation using point-cluster distance.")

# Dimension Reduction Parameters
parser.add_argument("--out_dims", type=int, action="store", default=2,
                        help="Output dimension")
parser.add_argument("--perp", type=int, nargs="+", action="store", default=30,
                        help="Perplexity or perplexity list for t-SNE.")
parser.add_argument("--early_exaggeration", type=float, action="store", default=12.0,
                        help="Early exaggeration factors for t-SNE.")
parser.add_argument("--max_iter", type=int, action="store", default=1000,
                        help="Max iteration for t-SNE.")
parser.add_argument("--early_exaggeration_iter", type=int, action="store", default=250,
                        help="Iterations of early exaggeration.")
parser.add_argument("--init", action="store", default="random",
                        help="Initialization method for t-SNE.")
parser.add_argument("--tsne_learning_rate", action="store", default="auto",
                        help="Learning rate for t-SNE.")
parser.add_argument("--open_tsne_method", action="store", default="fft",
                        help="Method for openTSNE.")
parser.add_argument("--dist_metric", type=str, action="store", default="euclidean",
                        help="Distance metric for applicable methods.")

#UMAP
parser.add_argument("--umap_min_dist", type=float, action="store", default=0.1,
                        help="min_dist for UMAP.")
parser.add_argument("--umap_neighbors", type=int, action="store", default=15,
                        help="Number of neighbors for UMAP.")

#SAUCIE
parser.add_argument("--SAUCIE_lambda_c", type=float, action="store", default=0,
                        help="Information dimension regularization for SAUCIE.")
parser.add_argument("--SAUCIE_lambda_d", type=float, action="store", default=0,
                        help="Intracluster distance regularization for SAUCIE.")
parser.add_argument("--SAUCIE_learning_rate", type=float, action="store", default=0.001,
                        help="Learning rate for SAUCIE.")
parser.add_argument("--SAUCIE_steps", type=int, action="store", default=1000,
                        help="Maximum iteration for SAUCIE.")
parser.add_argument("--SAUCIE_batch_size", type=int, action="store", default=256,
                        help="Batch size for SAUCIE.")

# KernelPCA
parser.add_argument("--kernelPCA", type=str, action="store", default="poly",
                        help="Kernel for kernel PCA.")

# PHATE
parser.add_argument("--phate_decay", type=int, action="store", default=40,
                        help="Alpha decay rate for PHATE.")
parser.add_argument("--phate_knn", type=int, action="store", default=5,
                        help="KNN for PHATE.")
    

def parse(args: Optional[List[str]]=None) -> Dict[str, Optional[str]]:
    # Parse arguments
    arguments = parser.parse_args(args)
    arguments = vars(arguments)
    
    if arguments["out"] is not None:
        arguments["out"] = new_dir(arguments["out"], no_new_dir=arguments["no_new_dir"])
        
    if arguments["save_downsample_index"]:
        assert arguments["out"] is not None
        index_out: str = arguments["out"] + "/index"
        arguments["save_downsample_index"] = new_dir(index_out, False)
...
def new_dir(path:str, no_new_dir:bool):
